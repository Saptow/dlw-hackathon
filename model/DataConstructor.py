...
class DatasetConstructor(data.Dataset):
    def __init__(self,
        ...
        for i in range(self.train_num):
            ...
            img = Image.open(os.path.join(self.data_root, img_name)).convert("RGB")
            gt_map = Image.fromarray(np.squeeze(np.load(os.path.join(self.gt_root, gt_map_name))))
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            ...
            random_h = random.randint(0, int((3 * img_shape[1]) / 4) - 1)
            random_w = random.randint(0, int((3 * img_shape[2]) / 4) - 1)
